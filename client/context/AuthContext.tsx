...
export type LiveToken = (typeof initialTokens)[number] & { timeSlot?: string };
...
export function getTimeSlotCapacity(centre: LiveCentre) {
  return Math.floor(centre.dailyWeighingCapacity / timeSlots.length);
}

export function getTimeSlotRemaining(centreId: string, date: string, timeSlot: string, tokens: LiveToken[], centres: LiveCentre[]) {
  const centre = centres.find((item) => item.id === centreId);
  if (!centre) return 0;
  const used = tokens.filter((token) => token.centreId === centreId && token.requestedDate === date && token.timeSlot === timeSlot && statusTokens.includes(token.status)).reduce((sum, token) => sum + token.expectedQuantity, 0);
  return Math.max(0, getTimeSlotCapacity(centre) - used);
}
...
type AuthContextValue = {
  ...
  requestToken: (input: { farmerId: string; centreId: string; requestedDate: string; timeSlot: string; expectedQuantity: number }) => TokenActionResult;
...
export function AuthProvider({ children }: { children: ReactNode }) {
  ...
  const value = useMemo<AuthContextValue>(() => ({
    ...
    bookSlot: async ({ farmerId, centreId, intendedQuantity }) => { await wait(); if (currentUser?.role !== "farmer") return { ok: false, error: "Please sign in as a farmer to book a Slot." }; if (intendedQuantity <= 0) return { ok: false, error: "Enter a quantity greater than zero." }; const centre = centres.find((item) => item.id === centreId); if (!centre) return { ok: false, error: "Choose a procurement centre to continue." }; const nextStart = parseDateLabel(todayLabel); const nextEnd = new Date(nextStart); nextEnd.setUTCDate(nextEnd.getUTCDate() + 8); setSlot({ ...initialSlot, id: `slot-${Date.now()}`, farmerId, centreId, intendedQuantity, startDate: formatDateLabel(nextStart), endDate: formatDateLabel(nextEnd), daysRemaining: 9, status: "Active" }); notify("Slot successfully booked", `Your slot is valid until ${formatDateLabel(nextEnd)}.`, farmerId); return { ok: true }; },
    requestToken: ({ farmerId, centreId, requestedDate, timeSlot, expectedQuantity }) => { if (currentUser?.role !== "farmer") return { ok: false, error: "Please sign in as a farmer to request a Token." }; const remaining = farmer.verifiedLandArea * policy.procurementLimitPerAcre - records.filter((item) => item.farmerId === farmerId || !item.farmerId).reduce((sum, item) => sum + item.quantity, 0); const visitDate = parseDateLabel(requestedDate); const today = parseDateLabel(todayLabel); const slotStart = parseDateLabel(slot.startDate); const slotEnd = parseDateLabel(slot.endDate); if (slot.status !== "Active") return { ok: false, error: "You need an active Slot before booking a Token." }; if (visitDate <= today || visitDate < slotStart || visitDate > slotEnd) return { ok: false, error: "Choose a visit date inside your active 9-day Slot, at least one day from today." }; if (!timeSlots.some((item) => item.id === timeSlot)) return { ok: false, error: "Choose a valid time slot." }; if (expectedQuantity <= 0 || expectedQuantity > remaining) return { ok: false, error: `Expected quantity must be between 1 and ${remaining} quintals.` }; const availability = getDateAvailability(centreId, requestedDate, tokens, centres); if (availability === "Full") return { ok: false, error: "This date is fully booked. Please choose another day within your Slot validity." }; const dateUsed = tokens.filter((item) => item.centreId === centreId && item.requestedDate === requestedDate && statusTokens.includes(item.status)).reduce((sum, item) => sum + item.expectedQuantity, 0); const centre = centres.find((item) => item.id === centreId); if (!centre || expectedQuantity > centre.dailyWeighingCapacity - dateUsed) return { ok: false, error: "This request exceeds the remaining capacity for the selected date." }; const timeRemaining = getTimeSlotRemaining(centreId, requestedDate, timeSlot, tokens, centres); if (expectedQuantity > timeRemaining) return { ok: false, error: "This time slot does not have enough remaining capacity." }; const dailyCount = tokens.filter((item) => item.centreId === centreId && item.requestedDate === requestedDate).length; const next = { id: `token-${Date.now()}`, tokenNumber: `TKN-${centreId}-${requestedDate.replace(/\s/g, "-")}-${String(dailyCount + 1).padStart(4, "0")}`, shortReference: `T-${String(454 + dailyCount).padStart(4, "0")}`, slotId: slot.id, farmerId, centreId, requestedDate, timeSlot, expectedQuantity, status: "Waiting for Approval" as TokenStatus, batchId: null, createdAt: "Just now", approvedAt: null, completedAt: null } as LiveToken; setTokens((items) => [...items, next]); notify("Token requested", "Your request is being reviewed by the Procurement Officer.", farmerId); return { ok: true, token: next }; },
    updateTokenStatus: async (tokenId, status) => { await wait(); if (currentUser?.role !== "officer") return { ok: false, error: "Access denied — insufficient permissions." }; const token = tokens.find((item) => item.id === tokenId); if (status === "Approved" && token) { const availability = getDateAvailability(token.centreId, token.requestedDate, tokens.filter((item) => item.id !== tokenId), centres); if (availability === "Full") return { ok: false, error: "Daily capacity exceeded." }; } setTokens((items) => items.map((item) => item.id === tokenId ? { ...item, status, approvedAt: status === "Approved" ? "Just now" : item.approvedAt } : item)); if (token) notify(status === "Approved" ? "Token approved" : `Token ${status.toLowerCase()}`, status === "Approved" ? "You can come now." : `Your token is now ${status}.`, token.farmerId); return { ok: true }; },
